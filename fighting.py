...
def run(
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            cv2.imshow("demo", im0)
            cv2.imwrite("fighting.jpg",im0)
            if len(det):
                if names[c]=="Fighting":
                            bot.sendMessage('5446609180',str("Fighting detected"))
                            bot.sendPhoto('5446609180',photo=open("fighting.jpg","rb"))
...
def parse_opt(File):
    ...
    parser.add_argument('--weights', nargs='+', type=str, default=ROOT / 'fighting.pt', help='model path or triton URL')
