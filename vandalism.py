...
def run(
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                if names[c]=="Vandalism":
                            bot.sendMessage('5446609180',str("Vandalism detected"))
                            bot.sendPhoto('5446609180',photo=open("vandalisum.jpg","rb"))
...
def parse_opt(File):
    ...
    parser.add_argument('--weights', nargs='+', type=str, default=ROOT / 'Vandalism.pt', help='model path or triton URL')
