...
def run(
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            cv2.imshow("demo", im0)
            cv2.imwrite("weapon.jpg",im0)
            if len(det):
                bot.sendMessage('5446609180',str("weapon detected"))
                bot.sendPhoto('5446609180',photo=open("weapon.jpg","rb"))
...
def parse_opt(File):
    ...
    parser.add_argument('--weights', nargs='+', type=str, default=ROOT / 'weapon.pt', help='model path or triton URL')
