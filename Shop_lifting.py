...
def run(
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            cv2.imshow("demo", im0)
            cv2.imwrite("shop_lift.jpg",im0)
            if len(det):
                if names[c]=="abnormal_action":
                            bot.sendMessage('5446609180',str("abnormal_action detected"))
                            bot.sendPhoto('5446609180',photo=open("shop_lift.jpg","rb"))
...
def parse_opt(File):
    ...
    parser.add_argument('--weights', nargs='+', type=str, default=ROOT / 'shoplifting.pt', help='model path or triton URL')
